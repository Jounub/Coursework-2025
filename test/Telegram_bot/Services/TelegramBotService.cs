...
[ФИО преподавателя] - прямой поиск преподавателя";
        ...
        bool oneDaySchedule)
    {
        try
        {
            ...
            var schedule = await _scheduleParser.GetTeacherScheduleAsync(teacherId, teacherName, startOfWeek, endOfWeek);
        ...
        {
            _logger.LogError(ex, $"Teacher schedule load error for {teacherName}");
            await _botClient.SendTextMessageAsync(
                chatId: chatId,
        ...
        CancellationToken ct)
    {
        ...
        for (int day = 1; day <= daysInMonth; day++)
        {
            ...
            row.Add(InlineKeyboardButton.WithCallbackData(
                day.ToString(),
                $"{prefix}_{id}_{month}_{day}_0_{name}"));
        ...
        row.Add(InlineKeyboardButton.WithCallbackData(
            todayText,
            $"{todayPrefix}_{id}_{DateTime.Today.Month}_{DateTime.Today.Day}_1_{name}"));
        
        row.Add(InlineKeyboardButton.WithCallbackData(
            tomorrowText,
            $"{todayPrefix}_{id}_{DateTime.Today.Month}_{DateTime.Today.AddDays(1).Day}_1_{name}"));
